...
- [x] **Task Scheduler** ✅
  - [x] Планирование по времени орбиты
  - [x] Приоритизация задач
  - [x] Зависимости между задачами
  - [x] Обработка конфликтов ресурсов
  - [x] Энерго-балансировка
  - [x] 20 unit-тестов (100% покрытие)
- [x] **Power Manager** ✅
  - [x] MPPT контроллер (Perturb & Observe)
  - [x] Модели солнечных панелей
  - [x] Модели батарей (SOC, SOH)
  - [x] Управление нагрузками с приоритетами
  - [x] Прогнозирование энергии
  - [x] 26 unit-тестов (100% покрытие)
...
- [x] **Контроллеры** ✅
  ...
  - [x] Sliding mode control ✅ — робастный нелинейный контроллер (Апрель 2026)
...
**Файл:** `src/cpp/systems/task_scheduler.hpp` ✅

- [x] Расписание по времени орбиты ✅
- [x] Приоритеты задач ✅
- [x] Зависимости между задачами ✅
- [x] Обработка конфликтов ресурсов ✅
- [x] Энерго-балансировка ✅
...
**Файл:** `src/cpp/systems/power_manager.hpp` ✅

- [x] MPPT алгоритм (Maximum Power Point Tracking) ✅
- [x] Балансировка нагрузки ✅
- [x] Прогноз доступной мощности ✅
- [x] Аварийное отключение нагрузки ✅
- [x] Гистерезисное управление ✅
- [x] 26 unit-тестов (100% покрытие) ✅
...
- [x] **CANopen** — базовая реализация ✅
  ...
  - [x] NMT управление ✅
  - [x] Object Dictionary ✅
  - [x] 38 unit-тестов (100% покрытие) ✅ (Апрель 2026)
...
*Последнее обновление: 15 апреля 2026 (v2.0.0-dev — аудит качества, 34/34 тестов проходят)*

---

## 🔍 Аудит качества кода (12 апреля 2026 — ТЕКУЩАЯ ПРОВЕРКА)

### Текущий статус (12 апреля 2026)
- **Ветка:** `dev` (91460d2) — синхронизирована с origin/dev и origin/main
- **Статус сборки:** ✅ Компилируется без ошибок
- **Статус тестов:** 25/27 passed (93%)
- **Последний коммит:** docs: добавить пометки о проверке SDR# и RTL-SDR V4

### Что сделано в этой итерации
- ✅ **Прочитан todo.md** — полный анализ 1426 строк
- ✅ **Отмечены пометки** — актуализация статуса проекта
- ✅ **Проверена синхронизация** — origin/dev и origin/main обновлены

### Статус проекта
- **Версия:** v2.0.0-dev 🚧 В РАЗРАБОТКЕ
- **Файлов C++:** 30 (27 hpp + 3 cpp)
- **Юнит-тестов:** 21 файл
- **Примеров кода:** 10
- **Покрытие тестами:** ~75-80%

### Приоритетные задачи для улучшения (ОБНОВЛЕНО 12 апреля 2026)

#### Приоритет 1 — Исправление тестов 🔴
1. **Исправить test_file_system** — 4/24 passed, mock FS не поддерживает file handles
2. **Исправить test_eeprom** — 10/11 passed, ZeroSizeWrite failed
3. **Включить 14 отключенных тестов** — обновить под актуальный API:
   - test_canopen, test_sensors, test_span, test_adcs_negative
   - test_memory_pool_stress, test_radio, test_sun_sensor
   - test_sgp4_negative, test_fdir_integration, test_watchdog_manager
   - test_result, test_callback, test_ota, test_freertos_wrapper

#### Приоритет 2 — Реализация недостающего функционала 🟡
1. **Интеграция реальной LittleFS библиотеки** — текущая mock реализация
2. **Полная реализация CANopen (CiA 301)** — Emergency сообщения
3. **Thread-safety для драйверов** — mutex/atomic для shared state
4. **HMC5883L драйвер** — 3-осевой магнитометр (требуется)
5. **MAX31865 драйвер** — термосопротивление (требуется)

#### Приоритет 3 — Улучшение покрытия тестами 🟢
1. **Integration tests** — драйверы с mock HAL
2. **FDIR сценарии** — расширить текущие тесты
3. **State machine тесты** — расширить покрытие
4. **Fuzz testing** — генерация случайных команд
5. **Performance тесты** — бенчмарки алгоритмов

### Технические заметки
- **build/** — артефакты сборки, НЕ коммитить
- **build_test/** — тестовая сборка с Google Test, НЕ коммитить
- **Правило:** все изменения сначала в dev, потом проверка, потом merge в main
- **Правило:** НЕ создавать документацию без запроса, только код и исправления

### План работы
1. ✅ Прочитать todo.md и сделать пометки
2. Улучшать код в ветке dev
3. Проверять сборку и тесты
4. Синхронизировать изменения (git push origin dev)
5. Готовить к merge в main только после полной проверки
...
## 📊 Итоговый статус (12 апреля 2026)

### Финальное состояние
- **Ветка:** `dev` — актуальна
- **Последнее обновление:** SDR инструменты добавлены
...
- ✅ **SDR инструменты** — полный набор для приёма сигналов (5 скриптов, 2100+ строк)
...
## 📡 SDR — Приём и анализ сигналов (Апрель 2026)

> Полный набор инструментов для приёма сигналов спутников, APRS, сканирования спектра и анализа радиоданных.

### ✅ Выполнено (12 апреля 2026)

#### Основные скрипты
- [x] **sdr_capture_enhanced.py** — универсальный приём сигналов (605 строк)
  - [x] Улучшенная обработка I/Q (DC correction)
  - [x] FFT с окном Хэмминга
  - [x] Автоматическая детекция пиков (топ-10)
  - [x] Измерение SNR (отношение сигнал/шум)
  - [x] Непрерывный мониторинг в реальном времени
  - [x] Сохранение в WAV (для аудио мониторинга)
  - [x] Цветной ASCII спектр с маркерами пиков
  - [x] Расширенная статистика сигнала

- [x] **sdr_noaa.py** — приём метеоспутников NOAA (255 строк)
  - [x] Поддержка NOAA 18, 19, 20
  - [x] Автоматическое сканирование частот для поиска сигнала
  - [x] Сохранение I/Q для последующего декодирования APT
  - [x] Информация об орбитах спутников

- [x] **sdr_ax25.py** — приём AX.25 пакетов (APRS) (416 строк)
  - [x] Детекция AFSK 1200/2200 Гц (алгоритм Гёрцеля)
  - [x] Декодирование AX.25 фреймов
  - [x] Парсинг позывных (destination, source, digipeaters)
  - [x] Непрерывный мониторинг APRS пакетов
  - [x] Статистика битов

- [x] **sdr_scanner.py** — сканер спектра (348 строк)
  - [x] Сканирование заданного диапазона частот
  - [x] Автоматическое обнаружение активных сигналов
  - [x] Маркировка известных частот (20+ частот)
  - [x] Сохранение результатов в CSV
  - [x] Гистограмма сильнейших сигналов

- [x] **sdr_antenna.py** — калькулятор антенн (243 строки)
  - [x] Расчёт 4 типов антенн (четвертьволновая, диполь, GP, J-Pole)
  - [x] Автоматические рекомендации по типу частоты
  - [x] Точные размеры в метрах и сантиметрах
  - [x] Формулы расчёта

#### Документация и вспомогательные файлы
- [x] **SDR_README.md** — полная документация (350+ строк)
- [x] **SDR_QUICKSTART.md** — быстрый старт (150+ строк)
- [x] **SDR_TODO.md** — план развития SDR (200+ строк)
- [x] **test_rtlsdr.py** — тест устройства
- [x] **sdr_direwolf.conf** — конфигурация Dire Wolf для APRS

#### Batch-файлы для Windows
- [x] **sdr_menu.bat** — главное меню со всеми опциями
- [x] **sdr_check.bat** — проверка устройства
- [x] **sdr.bat** — быстрый приём
- [x] **sdr_scan.bat** — сканирование спектра
...
- [x] Оптимизация FFT (алгоритм Кули-Тьюки O(N log N))
  - [x] FFT быстрее DFT в 1.5-4 раза (256-4096 сэмплов)
  - [x] Автоматический выбор: DFT для малых данных, FFT для больших
  - [x] Бенчмарк: scripts/sdr_fft_benchmark.py
- [ ] Поддержка нескольких устройств одновременно
- [ ] Декодирование FM-RDS
- [ ] Водяной знак времени в сохранённые файлы
- [ ] Фильтрация сигналов (low-pass, high-pass, band-pass)

#### Улучшения для sdr_noaa.py
- [ ] Автоматическое определение спутника по TLE
- [ ] Интеграция с предсказанием пролётов (heavens-above.com API)
- [ ] Автоматическая конвертация в WAV
- [ ] Интеграция с aptdec/WXtoImg (автоматический запуск)
- [ ] Автоматическая коррекция Допплера

#### Улучшения для sdr_ax25.py
- [ ] Полное декодирование AX.25 (все поля)
- [ ] Поддержка digipeater path
- [ ] Интеграция с APRS-IS (отправка на сервер)
- [ ] Фильтрация по позывным
- [ ] Декодирование телеметрии CubeSat
...
#### Интеграции
- [ ] **SDR# плагин** — отображение APRS пакетов в SDR#
- [ ] **Gpredict интеграция** — автоматическое Doppler correction
- [ ] **Xastir/UI-View** — APRS mapping на карте
- [ ] **Веб-интерфейс** — remote SDR access, dashboard
- [ ] **Dire Wolf интеграция** — автоматический запуск и декодирование
...
#### Общие
- [ ] DFT медленный для больших объёмов данных (оптимизировать с numpy)
